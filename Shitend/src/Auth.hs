...
newProfile :: Validate Email -> Validate Password -> Validate Person
newProfile vEmail vPassword = liftA2 emptyProfile (fromEmail <$> vEmail) (fromPassword <$> vPassword)
  where 
    emptyProfile email password = Person
      { pid = def
      , email = email
      , password = password
